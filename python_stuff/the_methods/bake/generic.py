...
def generic(image_name, image_size, print_label, target_node_label, bake_type="EMIT", color_space="sRGB",
    ...
    for tile_y in range(num_tiles[1]):
        for tile_x in range(num_tiles[0]):
            ...
            m = mathutils.Matrix.Translation((
                -(tile_x*constants.bake.CYCLES_STITCHING_TILESIZE)/float(size[0]),
                -(tile_y*constants.bake.CYCLES_STITCHING_TILESIZE)/float(size[1]),
                0.0
            ))
    ...
    if constants.bake.CYCLES_STITCHING:
        ...
        final_image.save(constants.texture.TEXTURE_BAKED_FOLDER + image_name)
    ...
    bpy.ops.image.open(
        ...
        filepath=constants.texture.TEXTURE_BAKED_FOLDER + image_name,
        relative_path=True
    )
    ...
    if notifications.constants.ENABLED: notifications.constants.HANDLER.add_notification(
        "\U00002714 Done baking generic '" + print_label + "'", is_silent=False)
...
def albedo(skip_pp=False):
    img = generic(
        ...
        "Albedo", "AlbedoAO", skip_pp=skip_pp
    )
...
def specular(skip_pp=False):
    img = generic(
        ...
        "Specular", "SpecularAO", skip_pp=skip_pp
    )
    if constants.other.VERBOSE_LEVEL >= 1:
        common.general.safe_print(" - Done. Saving texture externally then deleting internally...")
    if not skip_pp:
...
def smoothness(skip_pp=False):
    img = generic(
        ...
        "Smoothness", "SmoothnessAO", skip_pp=skip_pp
    )
    if constants.other.VERBOSE_LEVEL >= 1:
        common.general.safe_print(" - Done. Saving texture externally then deleting internally...")
    if not skip_pp:
...
def emissions(skip_pp=False):
    img = generic(
        ...
        "Emissions", "EmissionsAO", fill_color=(0.0, 0.0, 0.0, 0.0), skip_pp=skip_pp
    )
    if constants.other.VERBOSE_LEVEL >= 1:
        common.general.safe_print(" - Done. Saving texture externally then deleting internally...")
    if not skip_pp:
